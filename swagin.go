...
func (g *SwaGin) StartGracefulWithLimit(limit int, addr ...string) (*http.Server, *LimitListener, error) {
	g.init()
	for _, s := range g.subApps {
		s.init()
	}
	...
	limitListener := NewLimitListener(listener, limit)
	go func() {
	...
	return server, limitListener, nil
}
